...
uint32_t PeripheralComponentInterconnectController::Read(uint16_t bus, uint16_t device, uint16_t function, uint16_t registeroffset) {
    uint32_t id = 
        0x1 << 31
        | ((bus & 0xFF) << 16)
        | ((device & 0x1F) << 11)
...
void PeripheralComponentInterconnectController::Write(uint16_t bus, uint16_t device, uint16_t function, uint16_t registeroffset, uint32_t value) {
    uint32_t id = 
        0x1 << 31
        | ((bus & 0xFF) << 16)
        | ((device & 0x1F) << 11)
    ...
    for (int bus = 0; bus < 8; bus++) {
        for (int device = 0; device < 32; device++) {
            ...
            for (int function = 0; function < numFunctions; function++) {
                ...
                if (dev.vendor_id == 0x0000 || dev.vendor_id == 0xFFFF) { //no more functions
                    break;
                }
